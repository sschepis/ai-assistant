...
const yaml_front_matter_1 = __importDefault(require("yaml-front-matter"));
        ...
        compile_talkdown_code: {
            action: ({ path }) => __awaiter(void 0, void 0, void 0, function* () {
                ...
                function readAndParseMD(filePath) {
                    ...
                    return yaml_front_matter_1.default.loadFront(content);
                }
                ...
                function compileDirectory(dirPath) {
                    ...
                    for (const entry of entries) {
                        if (entry.isDirectory()) {
                            items = items.concat(compileDirectory(path_1.default.join(dirPath, entry.name)));
                        }
                        else if (entry.isFile() && entry.name.endsWith('.md')) {
                            const itemData = readAndParseMD(path_1.default.join(dirPath, entry.name));
                            items.push({
                                name: itemData.title || path_1.default.basename(entry.name, '.md'),
                ...
                function compile(projectPath) {
                    ...
                    const configPath = path_1.default.join(projectPath, 'config.json');
                    if (fs_1.default.existsSync(configPath)) {
                        compiled.config = JSON.parse(fs_1.default.readFileSync(configPath, 'utf8'));
                    }
                    const dirs = ['functions', 'directives', 'processes', 'utils'];
                    dirs.forEach(dir => {
                        const dirPath = path_1.default.join(projectPath, 'src', dir);
