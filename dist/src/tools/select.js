...
exports.default = {
    schema: { "name": "select", "description": "Performs the selector operation on the HTML page at the given path. The operation can be get, append, prepend, replace, remove, get_attributes, or set_attributes, or summarize. IF running in the browser, the path is ignored and the current page is used.", "input_schema": { "type": "object", "properties": { "path": { "type": "string", "description": "The file path to the HTML file." }, "operation": { "type": "string", "description": "The operation to perform on the selector. Can be get, append, prepend, replace, remove, get_attributes, set_attributes, or summarize" }, "selector": { "type": "string", "description": "The CSS selector to match elements." }, "value": { "type": "string", "description": "The HTML content to append." }, "n": { "type": "string", "description": "For summarize, specifies the depth of child elements to summarize. 0 for detailed information." } }, "required": ["selector", "operation"] } },
    action: function ({ path, operation, selector, value, n }) {
        let dom;
        try {
            if (typeof document !== 'undefined') {
                ...
                const html = fs_1.default.readFileSync(path, 'utf8');
                dom = new jsdom_1.JSDOM(html).window.document;
            }
        }
        catch (e) {
            const html = fs_1.default.readFileSync(path, 'utf8');
        ...
        fs_1.default.writeFileSync(path, dom.serialize());
